...
const cadastrarUsuario = async (req, res) => {
    ...
    try {
        ...
        const obterEmail = await knex("usuarios").where("email",email).debug();
        ...
        const usuario = await knex("usuarios").insert(dadosUsuarios).debug();
...
const atualizarPerfil = async (req, res) => {
    ...
    const update = await knex('usuarios').update(body).join(', ').where('id',req.usuario.id).debug();
